...
//	}
	...
	private static void _printNodeInfo2(StringBuffer sb, IRModel model, List<IRReteNode> nodes) throws RException {
		...
		List<Integer> maxLen = toList(8, 6, 14, 36, 30);

		result.add(toList("NODE[n]", "Queue", "Parent", "Child", "Rule", "Inherit"));
		...
		for (IRReteNode node : nodes) {
			...
			result.add(toList(node.getNodeName() + "[" + node.getEntryLength() + "]", queueType,
					...
					ruleNames.isEmpty() ? null : "" + ruleNames, inheritExpr));
		}
	...
	private static void _printNodeInfo3(StringBuffer sb, IRModel model, List<IRReteNode> nodes) throws RException {
		...
		for (IRReteNode node : nodes) {
			...
			case RULE:
				description = RulpUtil.toString(((XRRuleNode) node).getActionStmtList());
				break;

			default:
				description = node.getUniqName();
				break;
			}

			String des2 = node.getMatchDescription();
			if (des2 != null) {
				description += " " + des2;
			}

			sb.append(String.format("%8s  %s\n", node.getNodeName() + "[" + node.getEntryLength() + "]", description));
		}
	...
	private static void _printReteCounter(StringBuffer sb, IReteNodeMatrix nodeMatrix, boolean isRule)
		...
		if (!isRule) {
			...
			sb.append(SEP_LINE1);
			sb.append(" NODE         Source        UniqName\n");
			sb.append(SEP_LINE2);
